...
// IIQES BatchCheckIn Enlarge Text Feature
chrome.webRequest.onCompleted.addListener(function (details) {
  log("https://owensboro.incidentiq.com/api/v1.0/assets/search Triggered")
  if (details.statusCode != 200)
    return

  chrome.storage.sync.get(Config.DEFAULT_OPTIONS).then((settings) => {
    if (!settings.IIQES_BATCHCHECKIN_ENLARGETEXT) {
      log("IIQES_BatchCheckIn_EnlargeText Not Enabled")
      return
    }
    ...
    getCurrentTab().then(async function (currentTab) {
      await chrome.tabs.sendMessage(currentTab.id,
        {
          action: Message.IIQES_BATCHCHECKIN_ENLARGETEXT
        });
    })
  })
}, { urls: ["https://owensboro.incidentiq.com/api/v1.0/assets/search"] })
