...
chrome.runtime.onMessage.addListener(
    function (message, sender, sendResponse) {
        chrome.storage.sync.get(Config.DEFAULT_OPTIONS).then((settings) => {
            if (message.action == Config.RAPIDSCAN_CHECK && settings.IIQRS_SoundEffects) {
            ...
            } else if (message.action == Config.FOCUS_DEVICE_TEXTBOX && settings.IIQBatchChecKIn_AutoFocus) {
